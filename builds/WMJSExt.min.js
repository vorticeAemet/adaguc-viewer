...
R){function O(ma){return function(la){ba[ma]=la;ia--;0==ia&&T(ba)}}var ba=[],ia=0;do ba.push(void 0),ia++,x(G.value).callWhenSettled_(O(ba.length-1),R),G=P.next();while(!G.done)})};return w},"es6","es3");$jscomp.polyfill("Object.is",function(c){return c?c:function(n,x){return n===x?0!==n||1/n===1/x:n!==n&&x!==x}},"es6","es3");
...
R,S),S!=R&&c.parseLayer(!0,G(O,R))):(console.log("TIME",ba,L,O),L!=ba&&c.parseLayer(!0,G(O,void 0)))}}})}catch(T){console.log(e)}};c.getUpdateTime=function(y){return"PT1D"==y?216E5:"PT12H"==y?108E5:"PT6H"==y?72E5:"PT3H"==y?36E5:"PT1H30M"==y?18E5:"PT1H"==y?12E5:"PT30M"==y||"PT20M"==y?6E5:"PT15M"==y?3E5:"PT10M"==y?18E4:"PT5M"==y?12E4:"PT3M"==y||"PT1M"==y?6E4:null};c.getUpdateStatus=function(){return n};c.setUpdateStatus=function(y){n=y};c.opacitySliderChanged=function(y){c.WMJSLayer&&(c.WMJSLayer.setOpacity(y/
...
I18n.unable_to_connect_server.text+"<br/><br/>"+T)});w&&w(c);c.afterlayerready&&c.afterlayerready(c)}else{c.WMJSLayer.getLayerRelative(function(ma,la,ua){c.productButton.setText("("+(la+1)+"/"+ua+") "+c.WMJSLayer.title)},function(ma){c.productButton.setText(c.WMJSLayer.path+c.WMJSLayer.title)});c.setStyleText(c.WMJSLayer.getStyleObject(c.WMJSLayer.currentStyle));c.dimensionPanels=[];c.dimensionPanelContainer.removeAll();var R=sync_layer,O="";for(G=0;G<c.WMJSLayer.dimensions.length;G++){for(var ba=
...
c.WMJSLayer.parentMaps[0].rebuildMapDimensions();0==c.dimCallBackAdded&&(c.WMJSLayer.parentMaps[0].addListener("ondimchange",c.dimUpdate,!0),c.dimCallBackAdded=!0);try{c.styleButton.loadStyles()}catch(ma){}var ia=!1;isDefined(c.name)&&0<c.name.length&&(ia=!0);c.setTitle("Layer "+c.WMJSLayer.name);if(!1===ia){if(1==c.autoChooseLayer){c.autoChooseLayer=!1;c.WMJSLayer.autoSelectLayer(function(ma){c.setLayer(ma.name);c.WMJSLayer.zoomToLayer();ia=!0});return}c.productButton.showService()}c.WMJSLayer.WMJSService?
...
I18n.title.text+"</i></td><td> "+c.WMJSLayer.WMJSService.title+"</td></tr><tr><td><i>"+I18n.service.text+"</i></td><td>"+c.WMJSLayer.WMJSService.onlineresource+"</td></tr><tr><td><i>"+I18n.abstract.text+"</i></td><td>"+c.WMJSLayer.WMJSService.abstract+"</td></tr></table></span>"):c.layerAbstractPanel.update("...");G=function(){var ma=c.WMJSLayer.parentMaps[0].getLegendGraphicURLForLayer(c.WMJSLayer),la=c.WMJSLayer.parentMaps[0].getLegendStore();if(c.layerLegendPanel.getEl()){var ua=$(c.layerLegendPanel.getEl().dom).find("canvas")[0].getContext("2d");
(ma=la.getImageForSrc(ma))?(ua.rect(0,0,90,250),ua.fillStyle="white",ua.fill(),ua.drawImage(ma.getElement()[0],0,0)):(ua.rect(0,0,90,250),ua.fillStyle="gray",ua.fill())}};c.WMJSLayer.parentMaps[0].getListener().addToCallback("onlegendready",G,!0);G();void 0==c.updatetime&&(console.log("TIME UNDEFINED"),G=864E5,void 0!=c.WMJSLayer.getDimension("time")&&(G=c.WMJSLayer.getDimension("time").values,G=getUpdateTime(G.slice(G.lastIndexOf("/")+1))),console.log("PT",G),c.updatetime=G);null==x&&(console.log("Pongo el timer"),
...
S=Ext.create("Ext.data.Store",{fields:["title","srs","bbox"],data:{items:[]},proxy:{type:"memory",reader:{type:"json",root:"items"}}}),L=Ext.create("Ext.grid.Panel",{border:!1,store:S,columns:[{text:"Title",dataIndex:"title",flex:1},{text:"ESPG Code",dataIndex:"srs"}],listeners:{select:{fn:function(la,ua){la=ua.get("bbox");x=ua.get("srs");w=la;T.getComponent("column2").getComponent("areaPanel").getComponent("bboxtop").setValue(la.split(",")[3]);T.getComponent("column2").getComponent("areaPanel").getComponent("bbox").getComponent("bboxwest").setValue(la.split(",")[0]);
T.getComponent("column2").getComponent("areaPanel").getComponent("bbox").getComponent("bboxeast").setValue(la.split(",")[2]);T.getComponent("column2").getComponent("areaPanel").getComponent("bboxbottom").setValue(la.split(",")[1]);ua=Math.abs(w.split(",")[2]-w.split(",")[0]);la=Math.abs(w.split(",")[3]-w.split(",")[1]);var Da=c.coverages[0];T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("xresolution").setValue(ua/Da.width);T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("yresolution").setValue(la/
Da.height);ma()}}}}),P=Ext.create("Ext.data.Store",{fields:["title","format"],data:{items:[]},proxy:{type:"memory",reader:{type:"json",root:"items"}}}),G=Ext.create("Ext.grid.Panel",{border:!1,store:P,columns:[{text:I18n.format.text,dataIndex:"format"},{text:I18n.title.text,dataIndex:"title",flex:1}],listeners:{select:{fn:function(la,ua){y=ua.get("format");ma()}}}});c.onReady=function(){L.setLoading(!1);for(var la=this.coverages[0],ua=0;ua<la.supportedProjections.length;ua++){var Da=la.supportedProjections[ua].srs;
try{Da=epsgDescriptionLookup[la.supportedProjections[ua].srs]}catch(Fa){}isDefined(Da)||(Da=la.supportedProjections[ua].srs);S.add({title:Da,srs:la.supportedProjections[ua].srs,bbox:la.supportedProjections[ua].bbox})}ua=S.find("srs",la.nativeCRS);-1==ua&&(ua=S.find("srs","EPSG:4326"));-1==ua&&(ua=0);L.getView().select(ua);S.add({title:"Viewer window projection and area",srs:this.srs,bbox:this.bbox});T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("rasterwidth").setValue(la.width);
T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("rasterheight").setValue(la.height);Da=T.getComponent("column3").getComponent("dimensionPanel");for(ua=0;ua<la.dimensions.length;ua++)Da.add({xtype:"textfield",fieldLabel:la.dimensions[ua].name,value:la.dimensions[ua].currentValue,labelWidth:40,padding:"30",width:200,itemId:"dimtextfield_"+la.dimensions[ua].name,listeners:{change:{fn:function(){ma()}}}});for(ua=0;ua<la.formats.length;ua++){Da=la.formats[ua];
var va=la.formats[ua].toUpperCase();try{Da=formatDescriptionLookup[va]}catch(Fa){}isDefined(Da)||(Da=la.formats[ua]);P.add({title:Da,format:la.formats[ua]});"AAIGRID"==va&&G.getView().select(ua)}ma();ia.update('<table style="margin:5px;"><tr><td><b>Coverage:</b></td><td>'+la.name+"</td></tr><tr><td><b>Title:</b></td><td>"+la.title+"</td></tr><tr><td><b>Description:</b></td><td>"+la["abstract"]+"</td></tr></table>")};Ext.create("Ext.panel.Panel",{region:"west",border:!0,width:100});var T=Ext.create("Ext.panel.Panel",
...
itemId:"bboxtop",labelWidth:20,padding:"30 5 5 130",width:180,listeners:{change:{fn:function(){ma()}}}},{xtype:"panel",itemId:"bbox",border:!1,layout:"hbox",items:[{xtype:"textfield",fieldLabel:I18n.west.text,itemId:"bboxwest",labelWidth:30,padding:"15 0 0 15",width:180,listeners:{change:{fn:function(){ma()}}}},{xtype:"textfield",fieldLabel:I18n.east.text,itemId:"bboxeast",labelWidth:30,padding:"15 0 0 15",width:180,listeners:{change:{fn:function(){ma()}}}}]},{xtype:"textfield",fieldLabel:I18n.south.text,
itemId:"bboxbottom",labelWidth:40,padding:"15 5 5 110",width:200,listeners:{change:{fn:function(){ma()}}}}]},{xtype:"panel",region:"south",height:"50%",title:"Grid properties",split:!0,autoScroll:!0,itemId:"gridproperties",items:{xtype:"fieldcontainer",itemId:"field",items:[{xtype:"radiofield",boxLabel:"Cell size",name:"resolution",checked:!0,itemId:"radioselectcellsize",padding:"15 15 15 15",handler:function(la,ua){!0===ua&&(T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("rasterwidth").disable(),
T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("rasterheight").disable(),T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("xresolution").enable(),T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("yresolution").enable());ma()}},{xtype:"textfield",fieldLabel:"X resolution",padding:"0 0 0 40",itemId:"xresolution",width:170,listeners:{change:{fn:function(){ma()}}}},{xtype:"textfield",
fieldLabel:"Y resolution",padding:"0 0 0 40",itemId:"yresolution",width:170,listeners:{change:{fn:function(){ma()}}}},{xtype:"radiofield",boxLabel:"Raster size",name:"resolution",padding:15,handler:function(la,ua){!0===ua&&(T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("rasterwidth").enable(),T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("rasterheight").enable(),T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("xresolution").disable(),
T.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("yresolution").disable())}},{xtype:"textfield",fieldLabel:"Raster Width",padding:"0 0 0 40",disabled:!0,itemId:"rasterwidth",width:170,listeners:{change:{fn:function(){ma()}}}},{xtype:"textfield",fieldLabel:"Raster Height",padding:"0 0 0 40",disabled:!0,itemId:"rasterheight",width:170,listeners:{change:{fn:function(){ma()}}}}]}}]},{itemId:"column3",xtype:"panel",layout:"border",border:!1,split:!0,flex:1,items:[{xtype:"panel",
...
x);x.select();return x};function getUpdateTime(c){return"PT1D"==c?216E5:"PT12H"==c?108E5:"PT6H"==c?72E5:"PT3H"==c?36E5:"PT1H30M"==c?18E5:"PT1H"==c?12E5:"PT30M"==c||"PT20M"==c?6E5:"PT15M"==c?3E5:"PT10M"==c?18E4:"PT5M"==c?12E4:"PT3M"==c||"PT1M"==c?6E4:864E5}
...
0;x<getFeatureInfoApplications.length;n={$jscomp$loop$prop$location$106$329:n.$jscomp$loop$prop$location$106$329},x++)n.$jscomp$loop$prop$location$106$329=getFeatureInfoApplications[x],c.push({text:getFeatureInfoApplications[x].name,iconCls:getFeatureInfoApplications[x].iconCls,location:getFeatureInfoApplications[x].location,handler:function(O){return function(){console.log(O.$jscomp$loop$prop$location$106$329);y.setApplication(O.$jscomp$loop$prop$location$106$329);y.hide();y.show()}}(n)});var y=
...
iconCls:"button_stoploading",handler:function(){mainWebmapJS.webMapJS.abort()}}];try{isDefined(getFeatureInfoApplications)}catch(O){getFeatureInfoApplications=[]}var P={};for(x=0;x<getFeatureInfoApplications.length;P={$jscomp$loop$prop$location$108$331:P.$jscomp$loop$prop$location$108$331},x++)P.$jscomp$loop$prop$location$108$331=getFeatureInfoApplications[x],n.push({text:getFeatureInfoApplications[x].name,iconCls:getFeatureInfoApplications[x].iconCls,location:getFeatureInfoApplications[x].location,
handler:function(O){return function(){console.log(O.$jscomp$loop$prop$location$108$331);y.setApplication(O.$jscomp$loop$prop$location$108$331);y.hide();y.show()}}(P)}),getFeatureInfoApplications.open&&P.$jscomp$loop$prop$location$108$331.name===getFeatureInfoApplications.open&&y.setApplication(P.$jscomp$loop$prop$location$108$331);n.push({text:I18n.about_the_adaguc_viewer.text,iconCls:"button_info",tooltip:I18n.about_the_adaguc_viewer.tooltip,handler:function(){Ext.create("Ext.window.Window",{title:I18n.about_adaguc.text,
...
isDefined(O.bbox)&&mainWebmapJS.webMapJS.setProjection({srs:O.srs,bbox:O.bbox});if(isDefined(O.service)){clearLayers(mainWebmapJS.webMapJS);var ma=URLDecode(O.service).split(","),la=[""];isDefined(O.layer)?la=URLDecode(O.layer).split(","):1==ba&&(ia=!0);isDefined(O.selected);for(var ua=la.length,Da=0;Da<la.length;Da++)try{var va=la[la.length-1-Da].split("$"),Fa=va+"",Ma=!0,Ka="",Va=1,Ya=ma[0]+"";6==va.length&&(Fa=va[0],Ma=va[2],Ka=va[3],Va=va[4],isDefined(Ma)&&("true"===Ma&&(Ma=!0),"false"===Ma&&
(Ma=!1)),Ya=ma[va[5]]);createNewLayerPanel(mainWebmapJS.webMapJS,{service:Ya,name:Fa,style:Ka,enabled:Ma,opacity:Va,ready:function(ab,mb){ua--;if(0==ua){if(isDefined(O.dims))for(var Eb=O.dims.split(","),vb=0;vb<Eb.length;vb++){var sb=Eb[vb].split("$");2==sb.length&&mainWebmapJS.webMapJS.setDimension(sb[0],sb[1])}1==ba&&ab.WMJSLayer.zoomToLayer();mb.draw("index.html::numlayers=0")}},autoChooseLayer:ia})}catch(ab){error("Unable to parse layer nr "+Da)}if(isDefined(O.baselayers)){ma=O.baselayers.split("$");
ia=[];for(Da=la=0;Da<ma.length;Da++){va=!1;for(Fa=0;Fa<mapTypeConfiguration.length;Fa++)if(mapTypeConfiguration[Fa].baselayer.name==ma[Da]){mapTypeConfiguration[Fa].baselayer.enabled=!0;ia.push(mapTypeConfiguration[Fa].baselayer);va=!0;la++;break}if(!va)for(Fa=0;Fa<baseLayerConfiguration.length;Fa++)if(baseLayerConfiguration[Fa].name==ma[Da]){baseLayerConfiguration[Fa].enabled=!0;ia.push(baseLayerConfiguration[Fa]);va=!0;la++;break}va||error("Warning: unable to find baselayer "+ma[Da])}for(Da=0;Da<
baseLayerConfiguration.length;Da++)0==baseLayerConfiguration[Da].enabled?ia.push(baseLayerConfiguration[Da]):0==la&&ia.push(baseLayerConfiguration[Da]);baseLayers=[];for(Da=0;Da<ia.length;Da++)ma=new WMJSLayer(ia[Da]),0==Da&&(mainWebmapJS.baseLayer=ma),baseLayers.push(ma);0<baseLayers.length&&mainWebmapJS.webMapJS.setBaseLayers(baseLayers)}}O&&O.autowms&&(autowmsURL=O.autowms+"",-1===autowmsURL.indexOf("?")&&(autowmsURL+="?"),window.setTimeout(function(){y.setApplication(getFeatureInfoApplications.filter(function(ab){return"AutoWMS"===
ab.name})[0]);y.hide();y.show()},100));O&&O.wms&&(console.log("wms found",O.wms),createNewLayerPanel(mainWebmapJS.webMapJS,{service:O.wms}))};0==window.location.hash.length&&R(getUrlVars());checkIfHashTagChanged(function(O,ba){if(0<=O.indexOf("addlayer('")){if(ba=O.substring(O.indexOf("addlayer('")+10,O.indexOf("',")),O=O.substring(O.indexOf(",'")+2,O.indexOf("')")),ba&&O){for(var ia=!1,ma=mainWebmapJS.webMapJS.getLayers(),la=0;la<ma.length;la++)if(ma[la].service==ba&&ma[la].name==O){ia=!0;break}0==
ia&&(isDefined(hashLocationNotfiyAddLayer),firstTimeOpening=!0,createNewLayerPanel(mainWebmapJS.webMapJS,{service:ba,name:O,zoomtolayer:!1,notifyadded:!0}))}}else 0<=O.indexOf("positionmappinbylatlon(")?(debug(O),ba=O.substring(23,O.indexOf(")")),debug(ba),ba=ba.split(","),debug(ba[0]+" "+ba[1]),mainWebmapJS.webMapJS.positionMapPinByLatLon({x:parseFloat(ba[0]),y:parseFloat(ba[1])})):0<=O.indexOf("refreshlayers")?updateLayers():R(ba)});getFeatureInfoApplications.open&&y.show()});var gfiapp_d3c3=function(c,n){var x=function(R,O){$("#"+O).empty();$("#"+O).append("<div style='height=240px;border=2px solid blue;' class='chart'>a</div>");$("#"+O).append("<div style='height=40px;border=2px solid red;' class='controls gfiapp_d3c3_export_to_csv_button'>b</div>");var ba=$("#"+O).find(".chart");O=$("#"+O).find(".controls");ba.html("graph");O.button({label:"Export to CSV"}).click(function(){console.log("Export to CSV");for(var ia="",ma=R.datatoplot,la=0;la<ma[0].length;la++){for(var ua=
0;ua<ma.length;ua++)0<ua&&(ia+=";"),ia+=JSON.stringify(ma[ua][la]);ia+="\n\r"}ma=document.createElement("a");ma.setAttribute("download","export.csv");ma.setAttribute("href",encodeURI("data:text/csv;charset=utf-8,"+ia));console.log("Clicking link");document.body.appendChild(ma);ma.click();ma.remove()});c3.generate({bindto:ba.get(0),data:{x:"x",x_format:"%Y%m%d",columns:R.datatoplot,types:{x:"area"}},axis:{x:{type:"timeseries",tick:{format:R.timeFormat}},y:{label:R.ylabel}},zoom:{enabled:!0},subchart:{show:!1}})},
y=function(R,O){return R>O?1:R<O?-1:0},w=function(R,O){var ba;for(ba=0;ba<O.length;ba++)try{if(O[ba].getTime()===R.getTime())return ba}catch(ia){}return-1},S=function(R,O){var ba=1;"string"!=typeof R[0].dims&&(ba=R[0].dims.length);for(var ia=[],ma=function(va,Fa,Ma){var Ka=["x"];va=[Fa.name+" "+va];for(var Va in Ma){Fa=d3.time.format("%Y-%m-%dT%H:%M:%SZ").parse(Va);var Ya=parseFloat(Ma[Va]);Ka.push(Fa);va.push(Ya)}ia.push({date:Ka,value:va})},la=function(va,Fa,Ma,Ka){if(1==ba)ma("",Fa,Ma);else if(va+
2<ba)for(d in Ma)la(va+1,Fa,Ma[d],Ka+"["+d+"]");else for(d in Ma)ma("["+d+"]",Fa,Ma[d])},ua=0;ua<R.length;ua++)la(0,R[ua],R[ua].data,"");ua=function(va){for(var Fa=[],Ma=va.length,Ka=0;Ka<Ma;Ka++)for(var Va=va[Ka].date,Ya=1;Ya<Va.length;Ya++){var ab=Va[Ya],mb=w(ab,Fa);-1==mb&&Fa.push(ab)}Fa.sort(y);Fa.unshift("x");Va=[];for(Ya=0;Ya<Ma;Ya++)Va.push([]);for(Ka=0;Ka<Ma;Ka++)for(Va[Ka].push(va[Ka].value[0]),Ya=1;Ya<Fa.length;Ya++)mb=w(Fa[Ya],va[Ka].date),mb==Ya?(ab=va[Ka].value[mb],"nodata"==ab&&(ab=
...
ia+"]")}},error:function(ba,ia,ma){$("#"+O).html("Sorry! An error occurred while receiving JSON data from the server: ["+ma+"]");console.log("Error "+ma)}})},P=!0,G=[];G.set=!1;var T=function(R){if(0!=P){G.x=R.x;G.y=R.y;G.set=!0;var O=n.getLatLongFromPixelCoord({x:R.x,y:R.y});$("#chart").html('<img src="./img/ajax-loader.gif" alt="Loading..."/>');var ba=n.getLayers();if(isDefined(ba)){O="timeseries graph for location ["+Math.round(100*O.x)/100+","+Math.round(100*O.y)/100+"]<br/>";$("#chart").empty();
for(var ia=0;ia<ba.length;ia++){var ma=n.getWMSGetFeatureInfoRequestURL(ba[ia],R.x,R.y);ma=composeUrlObjectFromURL(ma);var la=ma.location+"?";for(key in ma.kvp){var ua=ma.kvp[key];"info_format"==key&&(ua="application/json");"time"==key&&(ua="1000-01-01T00:00:00Z/3000-01-01T00:00:00Z");"dim_member"==key&&(ua="*");"dim_ensemble_member"==key&&(ua="*");"dim_percentile"==key&&(ua="*");la+=key+"="+URLEncode(ua)+"&"}$("#chart").append("<div class='chartstyle' id='chart"+ia+"'></div>");$("#chart"+ia).html('<img src="./img/ajax-loader.gif" alt="Loading..."/>');
L(la,"chart"+ia)}$("#info").html(O)}else $("#info").html("No valid data received:<br/>"+data)}};n.addListener("mouseclicked",T,!0);this.enable=function(){c.html('<div id="info"></div><div id="chart" class="gfiapp_d3c3"></div>');$("#info").html("Click on the map to create a timeseries graph.");console.log("init");n.enableInlineGetFeatureInfo(!1);P=!0};this.disable=function(){P=!1;n.enableInlineGetFeatureInfo(!0);G.set=!1};this.resize=function(R,O){console.log("w="+R);$("#chart").width(R);G.set&&T(G)};
...
f?h>=h&&(f=g=h):(f>h&&(f=h),g<h&&(g=h)))}return[f,g]}function ma(a,b){var f=a._intern;a=a._key;a=a(b);return f.has(a)?f.get(a):b}function la(a,b){var f=a._intern;a=a._key;a=a(b);if(f.has(a))return f.get(a);f.set(a,b);return b}function ua(a,b){var f=a._intern;a=a._key;a=a(b);f.has(a)&&(b=f.get(b),f.delete(a));return b}function Da(a){return null!==a&&"object"===typeof a?a.valueOf():a}function va(a){return a}function Fa(a,b){for(var f=[],g=1;g<arguments.length;++g)f[g-1]=arguments[g];return Va(a,va,
...
f,g){f=void 0===f?0:f;g=void 0===g?b.length:g;for(g-=f=+f;g;){var h=a()*g--|0,k=b[g+f];b[g+f]=b[h+f];b[h+f]=k}return b}}function df(a){if(!(k=a.length))return[];for(var b=-1,f=Fc(a,$g),g=Array(f);++b<f;)for(var h=-1,k,p=g[b]=Array(k);++h<k;)p[h]=a[h][b];return g}function $g(a){return a.length}function ah(a){return a instanceof Set?a:new Set(a)}function ef(a,b){var f=a[Symbol.iterator]();a=new Set;var g={};b=$jscomp.makeIterator(b);for(var h=b.next();!h.done;g={$jscomp$loop$prop$value$336:g.$jscomp$loop$prop$value$336,
$jscomp$loop$prop$done$337:g.$jscomp$loop$prop$done$337},h=b.next())if(h=h.value,!a.has(h))for(g.$jscomp$loop$prop$value$336=void 0,g.$jscomp$loop$prop$done$337=void 0;function(k){return function(){var p=f.next();k.$jscomp$loop$prop$value$336=p.value;k.$jscomp$loop$prop$done$337=p.done;return p}}(g)();){if(g.$jscomp$loop$prop$done$337)return!1;a.add(g.$jscomp$loop$prop$value$336);if(Object.is(h,g.$jscomp$loop$prop$value$336))break}return!0}function gf(a){return a}function bh(a){return"translate("+
...
v=Array(q),z=0,C;m=Float64Array.from({length:q*q},b?function(U,W){return m[W%q][W/q|0]}:function(U,W){return m[W/q|0][W%q]});for(C=0;C<q;++C){for(var r=0,t=0;t<q;++t)r+=m[C*q+t]+a*m[t*q+C];z+=B[C]=r}C=(z=Hk(0,Ik-g*q)/z)?g:Ik/q;r=0;h&&F.sort(function(U,W){return h(B[U],B[W])});t={};F=$jscomp.makeIterator(F);for(var A=F.next();!A.done;t={$jscomp$loop$prop$i$177$342:t.$jscomp$loop$prop$i$177$342},A=F.next()){t.$jscomp$loop$prop$i$177$342=A.value;A=r;if(a){var E=Jh(~q+1,q).filter(function(U){return function(W){return 0>
W?m[~W*q+U.$jscomp$loop$prop$i$177$342]:m[U.$jscomp$loop$prop$i$177$342*q+W]}}(t));k&&E.sort(function(U){return function(W,da){return k(0>W?-m[~W*q+U.$jscomp$loop$prop$i$177$342]:m[U.$jscomp$loop$prop$i$177$342*q+W],0>da?-m[~da*q+U.$jscomp$loop$prop$i$177$342]:m[U.$jscomp$loop$prop$i$177$342*q+da])}}(t));E=$jscomp.makeIterator(E);for(var H=E.next();!H.done;H=E.next())H=H.value,0>H?(u[~H*q+t.$jscomp$loop$prop$i$177$342]||(u[~H*q+t.$jscomp$loop$prop$i$177$342]={source:null,target:null})).target={index:t.$jscomp$loop$prop$i$177$342,
startAngle:r,endAngle:r+=m[~H*q+t.$jscomp$loop$prop$i$177$342]*z,value:m[~H*q+t.$jscomp$loop$prop$i$177$342]}:(u[t.$jscomp$loop$prop$i$177$342*q+H]||(u[t.$jscomp$loop$prop$i$177$342*q+H]={source:null,target:null})).source={index:t.$jscomp$loop$prop$i$177$342,startAngle:r,endAngle:r+=m[t.$jscomp$loop$prop$i$177$342*q+H]*z,value:m[t.$jscomp$loop$prop$i$177$342*q+H]}}else for(E=Jh(0,q).filter(function(U){return function(W){return m[U.$jscomp$loop$prop$i$177$342*q+W]||m[W*q+U.$jscomp$loop$prop$i$177$342]}}(t)),
k&&E.sort(function(U){return function(W,da){return k(m[U.$jscomp$loop$prop$i$177$342*q+W],m[U.$jscomp$loop$prop$i$177$342*q+da])}}(t)),E=$jscomp.makeIterator(E),H=E.next();!H.done;H=E.next()){var M=H.value;H=void 0;t.$jscomp$loop$prop$i$177$342<M?(H=u[t.$jscomp$loop$prop$i$177$342*q+M]||(u[t.$jscomp$loop$prop$i$177$342*q+M]={source:null,target:null}),H.source={index:t.$jscomp$loop$prop$i$177$342,startAngle:r,endAngle:r+=m[t.$jscomp$loop$prop$i$177$342*q+M]*z,value:m[t.$jscomp$loop$prop$i$177$342*
q+M]}):(H=u[M*q+t.$jscomp$loop$prop$i$177$342]||(u[M*q+t.$jscomp$loop$prop$i$177$342]={source:null,target:null}),H.target={index:t.$jscomp$loop$prop$i$177$342,startAngle:r,endAngle:r+=m[t.$jscomp$loop$prop$i$177$342*q+M]*z,value:m[t.$jscomp$loop$prop$i$177$342*q+M]},t.$jscomp$loop$prop$i$177$342===M&&(H.source=H.target));H.source&&H.target&&H.source.value<H.target.value&&(M=H.source,H.source=H.target,H.target=M)}v[t.$jscomp$loop$prop$i$177$342]={index:t.$jscomp$loop$prop$i$177$342,startAngle:A,endAngle:r,
value:B[t.$jscomp$loop$prop$i$177$342]};r+=C}u=Object.values(u);u.groups=v;return p?u.sort(p):u}var g=0,h=null,k=null,p=null;f.padAngle=function(m){return arguments.length?(g=Hk(0,m),f):g};f.sortGroups=function(m){return arguments.length?(h=m,f):h};f.sortSubgroups=function(m){return arguments.length?(k=m,f):k};f.sortChords=function(m){return arguments.length?(null==m?p=null:(p=Hr(m))._=m,f):p&&p._};return f}function Lh(){this._x0=this._y0=this._x1=this._y1=null;this._=""}function Wc(){return new Lh}
...
b=g.next().value;g=g.next().value;f.set(b,g)}return f};$jscomp.inherits(dd,Map);dd.prototype.get=function(a){return Map.prototype.get.call(this,ma(this,a))};dd.prototype.has=function(a){return Map.prototype.has.call(this,ma(this,a))};dd.prototype.set=function(a,b){return Map.prototype.set.call(this,la(this,a),b)};dd.prototype.delete=function(a){return Map.prototype.delete.call(this,ua(this,a))};var bf=function(a,b){b=void 0===b?Da:b;var f=$jscomp.construct(Set,[],this.constructor);Object.defineProperties(f,
{_intern:{value:new Map},_key:{value:b}});if(null!=a)for(a=$jscomp.makeIterator(a),b=a.next();!b.done;b=a.next())f.add(b.value);return f};$jscomp.inherits(bf,Set);bf.prototype.has=function(a){return Set.prototype.has.call(this,ma(this,a))};bf.prototype.add=function(a){return Set.prototype.add.call(this,la(this,a))};bf.prototype.delete=function(a){return Set.prototype.delete.call(this,ua(this,a))};var vp=Array.prototype.slice,Xg=Math.sqrt(50),Yg=Math.sqrt(10),Zg=Math.sqrt(2),av=pe(Math.random),dh=
...
tddjs=function(c,n){var x=!0,y=[];y.set=!1;var w=[],S,L=0,P=function(T){var R,O,ba,ia,ma,la,ua,Da,va;return $jscomp.asyncExecutePromiseGeneratorProgram(function(Fa){console.log("TAC",S);ba=0==w.length?"":document.getElementById("info").innerHTML;document.getElementById("info").innerHTML=ba;if(0==x)return Fa.return();y.x=T.x;y.y=T.y;y.set=!0;R=n.getLatLongFromPixelCoord({x:T.x,y:T.y});O=n.getLayers();if(!isDefined(O))return $("#info").html("No valid data received:<br/>"+data),Fa.return();info=n.getLayers().getFeatureInfoUrl;
ba="";0==n.layers.length&&(ba="No layer: Load a valid layer.",document.getElementById("info").innerHTML=ba);ia=[];for(ma=0;ma<n.layers.length;ma++)la=n.layers[n.layers.length-ma-1].service,ua=n.layers[n.layers.length-ma-1].name,la.includes("TEMP")||la.includes("KRR")||la.includes("KRF")||la.includes("ASV_FCT")||"sond_station"==ua?ia.push(n.layers[n.layers.length-ma-1]):la.includes("ECMWF")||la.includes("HARMONIE")?ia.push(n.layers[n.layers.length-ma-1]):ua.includes("sond_station")&&(console.log("ENTRO POR SOND_STATION"),
ia.push(n.layers[n.layers.length-ma-1]));0==ia.length&&(ba="No Sounding valid Layer: Load a valid layer.",document.getElementById("info").innerHTML=ba);Da={};for(va in ia){console.log(va,ia.length);if(ia[va].service.includes("HARMONIE")||ia[va].service.includes("ECMWF")||ia[va].service.includes("TEMP")||ia[va].service.includes("KRR")||ia[va].service.includes("KRF")||ia[va].service.includes("ASV_FCT")||ia[va].name.includes("sond_station"))console.log("MYLAYER_TEXT",ia[va].name),Da.$jscomp$loop$prop$myLayer$277$344=
ia[va],getJSONdata(Da.$jscomp$loop$prop$myLayer$277$344,n,y.x,y.y,"text/plain",function(Ma){return function(Ka){if(null!=Ka){if(console.log(Ka),null!=Ka.data){document.getElementById("load").innerHTML="";var Va=Ka.meta;if(Ma.$jscomp$loop$prop$myLayer$277$344.service.includes("TEMP")||Ma.$jscomp$loop$prop$myLayer$277$344.name.includes("sond_station"))Va="Station";else if(Ma.$jscomp$loop$prop$myLayer$277$344.service.includes("ECMWF")||Ma.$jscomp$loop$prop$myLayer$277$344.service.includes("HARMONIE")){var Ya=
Ma.$jscomp$loop$prop$myLayer$277$344.service.split("=")[1];Ya=Ya.replace("&","");Va.model=Ya;Va.name=Ya;Va="MODEL"}else Va="SAT-PROFILE";var ab=L;L++;Ya=window.open(ab,"SOND"+ab,'itemId="sonda'+ab+'",toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,width=1040,height=600');Ya.myvar=Ka;var mb=Ya.document;mb.write(js);mb.close();w.push([Ya,"SOND"+ab]);mb=Va+" : "+Ka.meta.name;mb+="Profile for location ["+Math.round(100*R.x)/100+","+Math.round(100*R.y)/100+"]";ab='<div id="SOND'+ab+
'"class="sonding">Profile for location ['+Math.round(100*R.x)/100+","+Math.round(100*R.y)/100+"]";ab+=" - "+Va+" : "+Ka.meta.name+"</div>";Ya.document.title=mb;document.getElementById("info").innerHTML+=ab}}else document.getElementById("load").innerHTML="",window.alert("SOND: No valid data")}}(Da));Da={$jscomp$loop$prop$myLayer$277$344:Da.$jscomp$loop$prop$myLayer$277$344}}Fa.jumpToEnd()})};n.addListener("mouseclicked",P,!0);var G=function(){c.html('<div id="load"></div><div id="info"></div><div id="table"></div>');
$("#info").html("Click on the map to load a profile.<br/> A Sounding,ECMWF Model or IASI-KRR/F layer loaded is necessary");document.getElementById("info").addEventListener("click",function(T){T=T.target.getAttribute("id");console.log(T);for(var R=0;R<w.length;R++)if(w[R][1]==T&&!w[R][0].closed){w[R][0].focus();break}},!1);isDefined(S)&&null!=S||(S=window.setInterval(function(){for(var T=0;T<w.length;T++)if(w[T][0].closed){var R=document.getElementById(w[T][1]);console.log(R);w.splice(T,1);R.remove();
0==w.length&&$("#info").html("Click on the map to load a profile.<br/> A Sounding,ECMWF Model or IASI-KRR/F layer loaded is necessary")}},1E3),console.log(S));n.enableInlineGetFeatureInfo(!1)};this.enable=function(){G();x=!0};this.disable=function(){x=!1;n.enableInlineGetFeatureInfo(!0);y.set=!1;for(var T=0;T<w.length;T++)console.log("CLOSE"),w[T].close();window.clearInterval(S);w=[];L=0};this.resize=function(T,R){$("#mitdd").width(T);y.set&&P(y)};this.enable()};
try{WMJSExtApplications.Sondeo=tddjs,WMJSExtApplications.VertProfiles=tddjs}catch(c){}function createXHR(){try{return new XMLHttpRequest}catch(c){}try{return new ActiveXObject("Msxml2.XMLHTTP.6.0")}catch(c){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0")}catch(c){}try{return new ActiveXObject("Msxml2.XMLHTTP")}catch(c){}try{return new ActiveXObject("Microsoft.XMLHTTP")}catch(c){}return!1}
...
function getJSONdata(c,n,x,y,w,S){console.log("LAYER",c);var L=c.service.split("=")[1];L=L.replace("&","");document.getElementById("load").innerHTML+="Procesando<br>";document.getElementById("load").innerHTML+='<img src="./img/ajax-loader.gif" alt="Loading..."/>';document.getElementById("table").innerHTML="";var P=WMJScheckURL(c.service);P+="&SERVICE=WMS&REQUEST=GetFeatureInfo&VERSION="+c.version;P+="&LAYERS="+URLEncode(c.name);c.name.split(",");var G=P;w="";L.includes("ECMWF")&&(G+="&QUERY_LAYERS=mean_sea_level_pressure&INFO_FORMAT=application/json",
P+="&QUERY_LAYERS=geopotential_height,temperature,relative_humidity,wind_barbs_vectors&INFO_FORMAT=application/json");if(L.includes("TEMP")||c.name.includes("sond_station"))P+="&QUERY_LAYERS=p,z,t,td,windSpd,windDir,eVSS&INFO_FORMAT=application/json",G+="&QUERY_LAYERS=station_name,ps,alt,station_cname&INFO_FORMAT=application/json";if(L.includes("KRR")||L.includes("KRF")||L.includes("ASV_FCT"))P+="&QUERY_LAYERS=air_temperature,dew_point_temperature&INFO_FORMAT=application/json",G+="&QUERY_LAYERS=surface_air_pressure,surface_air_temperature,surface_dew_point_temperature&INFO_FORMAT=application/json";
...
P+=w,G+=w}catch(R){S(null,void 0)}debug('<a target="_blank" href="'+P+'">'+P+"</a>",!1);var T=c.service+"&SERVICE=WMS&REQUEST=getCapabilities&VERSION="+c.version;getMeta(G,function(R){if(null!=R)if(console.log("META",R),G.includes("TEMP")||G.includes("sond_station"))getData(T,R,P,function(O){null!=O?S({meta:R,data:O}):S(null)});else if(G.includes("KRR")||G.includes("KRF")||G.includes("ASV_FCT"))getData_IASI(T,R,P,function(O){null!=O?S({meta:R,data:O}):S(null)});else{if(G.includes("ECMWF")||G.includes("HARMONIE"))req_list=
...
var ba=(ff-R)/36E5;G=Object.keys(O).toString()}var ia=c.includes("ECMWF")||c.includes("HARMONIE")?T.toString():G.toString();ia=ia.split("T");var ma=ia[0].replaceAll("-","");ma=parseInt(ma);var la=parseInt(ia[1].substring(0,2)),ua;T="OBSERVACION";if(c.includes("KRR")||c.includes("KRF")||c.includes("ASV_FCT")){la=ia[1].substring(0,5);var Da="";T="SAT-PROFILE"}c.includes("KRR")&&(ua="sSHAI IASI-KRR");c.includes("KRF")&&(ua="sSHAI IASI-KRF");c.includes("ASV_FCT")&&(ua="ECMWF forecast");c.includes("ECMWF")&&
...
(Ma=w[ia].data[2][G]-273.15)}if("alt"==w[ia].name){if(isDefined(Ka))continue;var Ka=parseFloat(w[ia].data[G]);isNaN(Ka)&&(Ka=parseFloat(w[ia].data[0]))}"station_cname_backup"!=w[ia].name||isDefined(Da)||(Da=w[ia].data[G])}if(isDefined(ua)&&"nodata"!=ua){isNaN(Ka)&&(Ka=0);x={model:T,index:ua,date:ma,run:la,step:0,lon:L,lat:P,ps:va,zs:Ka,name:ua+" "+Da,ts:Fa,tds:Ma};if(c.includes("ECMWF")||c.includes("HARMONIE"))x={model:"ECMWF",index:"MODEL",date:ma,run:la,step:ba,lon:L,lat:P,ps:va,zs:0,name:"MODEL"};
...
for(O=R.next();!O.done;O=R.next()){T=O.value;console.log(T);if("air_temperature"==T.standard_name)var ba=T.data;if("dew_point_temperature"==T.standard_name)var ia=T.data}for(var ma in S){ind=S[ma];R=parseFloat(ind)/100;var la=Object.keys(ba[ind]);O=parseFloat(ba[ind][la]);O-=273.15;la=parseFloat(ia[ind][la]);la-=273.15;G.push({n:ma,p:R,z:NaN,t:O,td:la,wD:NaN,wS:NaN})}}y(G)}})})}
...
O.next()){dat=ia.value;if("p"==dat.name)var ma=dat.data;if("z"==dat.name)var la=dat.data;if("t"==dat.name)var ua=dat.data;if("td"==dat.name)var Da=dat.data;if("windSpd"==dat.name)var va=dat.data;if("windDir"==dat.name)var Fa=dat.data;if("eVSS"==dat.name)var Ma=dat.data}O=parseInt(T);isDefined(ma[O])||(O+=1);O=Object.keys(ma[O]);ia=y.length;for(var Ka in ma){var Va=ma[Ka],Ya=la[Ka],ab=ua[Ka],mb=Da[Ka],Eb=va[Ka],vb=Fa[Ka],sb=Ma[Ka];sb=sb[O];parseFloat(sb);Va=Va[O];Va=parseFloat(Va);Va/=ba;999999999<
Va||0>Va||(Ya=parseFloat(Ya[O]),ab=parseFloat(ab[O]),ab-=273.15,mb=parseFloat(mb[O])-273.15,Eb=parseFloat(Eb[O]),sb=vb=parseFloat(vb[O]),vb*=Math.PI/180,vb%=2*Math.PI,Ya<R||isNaN(ab)||isNaN(mb)||0>sb||(Va={n:ia,p:Va,z:Ya,t:ab,td:mb,wD:vb,wS:Eb},ia+=1,y.push(Va)))}}getDataN(c,n,x,y,w)})}else w(y)}var wsond=function(c){};var table="",wmjsond,yradar=function(c,n){function x(ba,ia){if(ba.length!=ia.length)return!1;for(var ma=0;ma<ba.length;ma++)if(!ia.includes(ba[ma]))return!1;for(ma=0;ma<ia.length;ma++)if(!ba.includes(ia[ma]))return!1;return!0}function y(){for(var ba=[],ia=0;ia<n.layers.length;ia++)n.layers[n.layers.length-ia-1].service.includes("YRAD")&&ba.push(n.layers[n.layers.length-ia-1].name);return ba}wmjsond=n;var w=!0,S="",L=[],P=!1,G=[];L.set=!1;var T=null,R=function(ba){P&&(T.close(),P=!1);var ia=[];ma=
"";document.getElementById("info").innerHTML=ma;if(0!=w)if(L.x=ba.x,L.y=ba.y,L.set=!0,n.getLatLongFromPixelCoord({x:ba.x,y:ba.y}),ba=n.getLayers(),isDefined(ba)){info=n.getLayers().getFeatureInfoUrl;var ma="";0==n.layers.length&&(ma="No layer: Load a valid YRADAR layer.",document.getElementById("info").innerHTML=ma);ba=[];for(ma=0;ma<n.layers.length;ma++)n.layers[n.layers.length-ma-1].service.includes("YRAD")&&ba.push(n.layers[n.layers.length-ma-1]);ma=null;for(var la in ba)ma=ba[la],null!=ma&&""!==
ma.getFeatureInfoUrl?!1===ma.queryable?n.featureInfoRequestReady("Layer is not queryable.",ma):(document.getElementById("table").innerHTML="",ma.name.includes("2D")?ia.push(ma):ma.name.includes("3D")&&ia.push(ma)):(ma="Wrong layer: Load a valid yradar layer.",document.getElementById("info").innerHTML=ma,P&&T.close());console.log("YR",ia);if(0<ia.length){T=window.open("YRADAR.html","YRADAR",'itemId="sond",toolbar=0,scrollbars=0,location=0,statusbar=0,menubar=0,resizable=1,width=800,height=600');for(la=
0;la<ia.length;la++)ma=ia[la],getCells(ma,n,L.x,L.y,function(ua){null!=ua&&(T.document.getElementById("info").insertAdjacentHTML("afterbegin",ua),T.focus())});P=!0}}else $("#info").html("No valid data received:<br/>"+data)};n.addListener("mouseclicked",R,!0);n.addListener("ondimchange",function(ba){return $jscomp.asyncExecutePromiseGeneratorProgram(function(ia){if(1==ia.nextAddress)return P&&T.close(),$("#tableinfo").html(""),$("#info").html(""),$("#table").html(""),document.getElementById("info").innerHTML=
"",document.getElementById("info").innerHTML='<img src="./img/ajax-loader.gif" alt="Loading..."/>',ia.yield(new Promise(function(ma){return setTimeout(ma,500)}),2);console.log("DIM REFRESH!!!");O();ia.jumpToEnd()})},!0);n.addListener("onimageload",function(ba){var ia;return $jscomp.asyncExecutePromiseGeneratorProgram(function(ma){ia=y();x(ia,G)||(console.log("IMG REFRESH!!!"),O());ma.jumpToEnd()})},!0);var O=function(){G=y();c.html('<div id="tableinfo"></div><div id="info"></div><div id="table"></div>');
$("#tableinfo").html("");$("#info").html("");$("#table").html("");var ba='<img src="./img/ajax-loader.gif" alt="Loading..."/>';document.getElementById("info").innerHTML=ba;var ia=[];for(ba=0;ba<n.layers.length;ba++)n.layers[n.layers.length-ba-1].service.includes("YRAD")&&ia.push(n.layers[n.layers.length-ba-1]);0==ia.length&&(ba="No YRADAR Layer: Load a valid YRADAR layer.",document.getElementById("info").innerHTML=ba);ba={};for(var ma in ia){var la=ia[ma];if(null!=la&&""!==la.getFeatureInfoUrl)if(!1===
la.queryable)n.featureInfoRequestReady("Layer is not queryable.",la);else if(la.name.includes("2D")||la.name.includes("3D")){var ua=ia[ma];ba.$jscomp$loop$prop$path$347="";ba.$jscomp$loop$prop$rad$348=ua.name.substr(-3,3);ba.$jscomp$loop$prop$dim$346="2D";ua.name.includes("3D")&&(ba.$jscomp$loop$prop$dim$346="3D");la=WMJScheckURL(ua.service).split("/");var Da=la[0];la=la[2];ba.$jscomp$loop$prop$path$347=Da+"//"+la+"/";getJSONdate(ua,n,"text/plain",function(va){return function(Fa){var Ma=Fa.substring(0,
Fa.indexOf("T"));Fa=Fa.substring(Fa.indexOf("T")+1,Fa.indexOf("Z")-2);S!=Ma+Fa&&(S=Ma+Fa);"3D"==va.$jscomp$loop$prop$dim$346?va.$jscomp$loop$prop$path$347+="yradar/"+Ma+Fa+"REG3D":ua.name.includes("NAC")?va.$jscomp$loop$prop$path$347+="yradar/"+Ma+Fa+"NAC2D":va.$jscomp$loop$prop$path$347+="yradar/"+Ma+Fa+"REG2D";$.ajax({url:va.$jscomp$loop$prop$path$347,dataType:"text",success:function(Ka){document.getElementById("info").innerHTML="";var Va=Ka.split(/\r?\n|\r/);Ka="<b> RADAR: "+va.$jscomp$loop$prop$rad$348+
"</b><br>";table="3D"==va.$jscomp$loop$prop$dim$346?"<b>YRADAR3D</b><br>":"<b>YRADAR2D</b><br>";if("COM"!=va.$jscomp$loop$prop$rad$348&&"NAC"!=va.$jscomp$loop$prop$rad$348){for(var Ya=[],ab=0;ab<Va.length;ab++)0===ab&&Ya.push(Va[ab]),Va[ab].includes(va.$jscomp$loop$prop$rad$348)&&Ya.push(Va[ab]);Va=Ya}Va=getTable(Va,va.$jscomp$loop$prop$rad$348,va.$jscomp$loop$prop$dim$346,n);$("#tableinfo").append(table+Ka+Va);$("#info").html("Click on cell num to zoom it.<br>")},error:function(Ka,Va,Ya){console.log(Ka,
Va,Ya);"No encontrado"==Ya||"Not find"==Ya?window.alert("No esta disponible el fichero: "+va.$jscomp$loop$prop$path$347):window.alert("Se ha producido un error al cargar"+va.$jscomp$loop$prop$path$347);document.getElementById("info").innerHTML="Error al cargar las caracteristicas de las celulas."}})}}(ba))}ba={$jscomp$loop$prop$dim$346:ba.$jscomp$loop$prop$dim$346,$jscomp$loop$prop$path$347:ba.$jscomp$loop$prop$path$347,$jscomp$loop$prop$rad$348:ba.$jscomp$loop$prop$rad$348}}n.enableInlineGetFeatureInfo(!1)};
...
L+("&X="+x)+("&Y="+y);c.version===WMSVersion.version130&&(L=L+("&I="+x)+("&J="+y));L+="&FORMAT=image/gif&STYLES=";try{L+="&"+n._getMapDimURL(c),w+=L}catch(P){S(null)}MakeHTTPRequest(w,function(P,G){if(null!=P)console.error(P),S(null);else{P=JSON.parse(G);var T;for(T in P){G=P[T];if("RADIOE"==G.name&&(!isDefined(R)||"nodata"==R[Object.keys(R)]))var R=G.data;if("NUPIX"==G.name)var O=G.data;if("LATCEN"==G.name)var ba=G.data;if("LONCEN"==G.name)var ia=G.data;if("ECTOP"==G.name)var ma=G.data;if("VIL"==
G.name)var la=G.data;if("id"==G.name&&(!isDefined(ua)||"nodata"==ua[Object.keys(ua)]||"nan"==ua[Object.keys(ua)]))var ua=G.data;if("ZMAX"==G.name&&(!isDefined(Da)||"nodata"==Da[Object.keys(Da)]||isNaN(parseFloat(Da[Object.keys(Da)]))))var Da=G.data;if("ZMED"==G.name)var va=G.data;if("ADV"==G.name)var Fa=G.data;if("RAY"==G.name)var Ma=G.data;if("POS"==G.name)var Ka=G.data;if("NEG"==G.name)var Va=G.data;if("RAYo"==G.name)var Ya=G.data;if("DISTMAX"==G.name)var ab=G.data;if("DISTME"==G.name)var mb=G.data;
if("RY/K"==G.name)var Eb=G.data;if("LATCENR"==G.name)var vb=G.data;if("LONCENR"==G.name)var sb=G.data;if("DIRN"==G.name)var Pc=G.data;if("V(KMH)"==G.name)var Dc=G.data}try{var ed=R[Object.keys(R)],Ec=O[Object.keys(O)];if(isDefined(ba))var Fc=ba[Object.keys(ba)];if(isDefined(ia))var Rc=ia[Object.keys(ia)];if(isDefined(ma))var qc=ma[Object.keys(ma)];if(isDefined(la))var rc=la[Object.keys(la)];var fd=Da[Object.keys(Da)],gd=va[Object.keys(va)],Dd=Ma[Object.keys(Ma)],hd=Ka[Object.keys(Ka)],Ed=Va[Object.keys(Va)],
...
L+("&X="+x)+("&Y="+y);c.version===WMSVersion.version130&&(L=L+("&I="+x)+("&J="+y));L+="&FORMAT=image/gif&STYLES=";try{L+="&"+n._getMapDimURL(c),w+=L}catch(P){S(null)}MakeHTTPRequest(w,function(P,G){if(null!=P)console.error(P),S(null);else{P=JSON.parse(G);var T;for(T in P){G=P[T];if("ID_backup"==G.name)var R=G.data;if("LATCEN"==G.name)var O=G.data;if("LONCEN"==G.name)var ba=G.data;if("DVCE"==G.name)var ia=G.data;if("DVGR"==G.name)var ma=G.data;if("ECTOP"==G.name)var la=G.data;if("ESPE"==G.name&&(!isDefined(ua)||
...
if("DIRN"==G.name)var vb=G.data;if("V(KMH)"==G.name)var sb=G.data}try{var Pc=O[Object.keys(O)],Dc=ba[Object.keys(ba)],ed=ia[Object.keys(ia)],Ec=ma[Object.keys(ma)],Fc=la[Object.keys(la)],Rc=ua[Object.keys(ua)];ia="0";isDefined(Da)&&(ia=Da[Object.keys(Da)]);var qc=va[Object.keys(va)],rc=Fa[Object.keys(Fa)],fd=Ma[Object.keys(Ma)],gd=Ka[Object.keys(Ka)];Ma="0";isDefined(Ya)&&(Ma=Ya[Object.keys(Ya)]);Ya="0";isDefined(ab)&&(Ya=ab[Object.keys(ab)]);var Dd=mb[Object.keys(mb)],hd=Eb[Object.keys(Eb)],Ed=vb[Object.keys(vb)],
...
tooltip:I18n.delete_selected_flag_tooltip.tooltip,renderTo:"webmapjs_div_selected_point_interest",cls:"webmapjs_button_delete_point_interest",handler:function(){delete_flag()}});$("#webmapjs_point_interest_name, #webmapjs_point_interest_lon, #webmapjs_point_interest_lat").keypress(function(ba){if(13==ba.which)return setflags(),!1});var O=function(){var ba=c.find(".gfiapp_point_interest");ba.empty();for(var ia=n.getLayers(),ma=0;ma<ia.length;ma++){var la=ia[ma],ua=jQuery("<div />",{css:{margin:8}});
ba.append(ua);ua.html("LAYER "+la.service)}x.resize(L,P)};O();n.addListener("onlayeradd",O,!0);n.addListener("mouseclicked",function(ba){if(0!=w){var ia=n.getLatLongFromPixelCoord({x:ba.x,y:ba.y});G=ia.x;T=ia.y;$("#webmapjs_point_interest_lon").val(ia.x.toFixed(5));$("#webmapjs_point_interest_lat").val(ia.y.toFixed(5));$("#webmapjs_point_interest_lon, #webmapjs_point_interest_lat").css("border","1px solid grey");ia=n.getLayers();ba=n.getWMSGetFeatureInfoRequestURL(ia[0],ba.x,ba.y).replace("&INFO_FORMAT=text/html",
"&INFO_FORMAT=application/json")+"&JSONP=featureInfoCallback";$.ajax({dataType:"jsonp",jsonpCallback:"featureInfoCallback",type:"GET",cache:!1,url:ba,success:function(ma){ma[1]&&"nodata"!==ma[1].data?($(".webmapjs_label_selected_point_interest").text(ma[1].data),$(".webmapjs_button_delete_point_interest").css("display","inline")):($(".webmapjs_label_selected_point_interest").text(I18n.default_selected_flag.text),$(".webmapjs_button_delete_point_interest").css("display","none"))},error:function(ma,
la,ua){debug(la+", "+ua)}})}},!0);$("#webmapjs_point_interest_name").on("input",function(ba){onChangeEvents("name",ba)});$("#webmapjs_point_interest_lon").on("input",function(ba){onChangeEvents("lon",ba);changeMapPin()});$("#webmapjs_point_interest_lat").on("input",function(ba){onChangeEvents("lat",ba);changeMapPin()});this.onChangeEvents=function(ba,ia){var ma=Ext.getCmp("webmapjs_add_point_interest");ma.getEl().dom.style.background="";ma=Ext.getCmp("webmapjs_delete_point_interest");ma.getEl().dom.style.background=
...
(ba+'">&#8635;&nbsp;<b>Refresh</b></span></span>')},T=function(O,ba){c.html("... working ... ");var ia=WMJSGetServiceFromStore(ba,xml2jsonrequestURL);ia.getLayerObjectsFlat(function(ma){var la=""+G(O,ba);la+="<span class='autowms_app_serviceabstract'><b>"+ia.title.replace(/!/g,"<br/>")+"</b><hr/>"+ia.abstract.replace(/!/g,"<br/>");if(ba&&(la+='<hr/>WMS: <a target="_blank" href="'+ba+'&service=WMS&request=GetCapabilities">'+ba+"</a>",1<ba.split("?").length)){var ua=new WMJSKVP(ba.split("?")[1]),Da=
ba.split("?")[0].replace("adagucserver","adagucopendap/");Da=Da.replace(/([^:]\/)\/+/g,"$1");ua.source&&(Da+=ua.source);ua.dataset&&(Da+=ua.dataset+"/");la=la+('<hr/>OpenDAP: <span class=\'autowms_app_opendaplinks\'><a target="_blank" href="'+Da+'">'+Da+'</a><a target="_blank" href="')+(Da+'.das">DAS</a><a target="_blank" href="')+(Da+'.dds">DDS</a></span>')}la+="</span>";c.html("... working ... ");y={};for(ua=0;ua<ma.length;ua++)la+=P(ma[ua],ba),y[ma[ua].name]=ma[ua];c.html(la);$(".autowms_app_layeritem").attr("onclick",
...
va.keyCode&&R("")})},function(ma){ma=G();ma=ma+"<span class='autowms_app_error_container'><span class='autowms_app_error_container_header'>Unable to get WMS GetCapabilities</span><a target='_blank' href=\""+(ba+'service=WMS&request=GetCapabilities">'+ba+"</a>");c.html(ma+"</span>");$(".autowms_app_fileitem_return").attr("onclick","").click(function(la){w=[];R("")})},!0)},R=function(O){O&&"undefined"!==O||(O="");var ba=$(".autowms_app_request_input").first().val();ba&&0<ba.length&&(x=ba.trim().split("?")[0],
x.endsWith("?")||(x+="?"));var ia=function(la){if(la.error)c.html("Error from server: "+la.error);else{y={};w[w.length-1]!==O&&w.push(O);var ua=G(la,O);c.html("... working ... ");la.result.sort(function(va,Fa){if(va.name<=Fa.name)return-1;if(va.name>Fa.name)return 1;if(va.name==Fa.name)return 0});for(var Da=0;Da<la.result.length;Da++)ua+=L(la.result[Da]),y[la.result[Da].name]=la.result[Da];c.html(ua);$(".autowms_app_fileitem_return").attr("onclick","").click(function(va){w.pop();R(w[w.length-1])});
$(".autowms_app_fileitem_refresh").attr("onclick","").click(function(va){var Fa=$(this).attr("name");$(".autowms_app_fileitem").remove();$(".autowms_app_layeritem").remove();setTimeout(function(){R(Fa)},100)});$(".autowms_app_fileitem").attr("onclick","").click(function(va){va=y[$(this).attr("name")];!1===va.leaf?R(va.path):T(la,va.adaguc)});$(".autowms_app_request_button").attr("onclick","").click(function(va){R("")});$(".autowms_app_request_input").keyup(function(va){13==va.keyCode&&R("")})}},ma=
x+"request=getfiles&path="+O;c.html('... Reading <a href="'+ma+'">'+ma+"</a> ... ");try{$.ajax({contentType:"application/json",crossDomain:!0,type:"GET",url:ma,success:ia,error:function(la){console.error("Request failed without jsonp, trying jsonp");ia({result:[]});$.ajax({dataType:"jsonp",contentType:"application/jsonp",crossDomain:!0,type:"GET",url:ma,success:ia,error:function(ua){S("Unable to do ajax call")}})}})}catch(la){console.error(la),S(la)}};this.enable=function(){c.html("<div>Initializing...</div>");
R("")};this.disable=function(){};this.resize=function(O,ba){}};try{WMJSExtApplications.AutoWMS=autowms_app}catch(c){};var hipatia_app=function(c,n){function x(ma){$.ajax({url:"./hipatia/project.ls",type:"GET",dataType:"text",success:function(la){datalist=la.split("\n");datalist=datalist.sort();datalist=datalist.reverse();for(la=0;la<datalist.length;la++)""!=datalist[la]&&L.push(datalist[la].trim());hipatiaURL&&""!==hipatiaURL||(hipatiaURL=L[0]);ia("")},error:function(la,ua,Da){console.log(Da)},cache:!1})}initWMJS();c.addClass("autowms_app_container");var y="https://geoservices.knmi.nl/autowms?";try{y=autowmsURL,
serverURL=autowmsURL.replace("autowms","adaguc-server")}catch(ma){}var w={},S=[],L=[],P=function(ma){var la=O();c.html(la+JSON.stringify(ma));$(".autowms_app_fileitem_return").attr("onclick","").click(function(ua){ia("")})},G=function(ma,la,ua){console.log("FOLD",la);$.ajax({url:ma,dataType:"text",success:function(Da){lines=Da.split("\n");for(var va=Da=!1,Fa=0;Fa<lines.length;Fa++)line=lines[Fa],line.includes("BEGIN "+la)&&(Da=!0),line.includes("END")&&(Da=!1),line.includes("<Layer")&&(va=!0),line.includes("</Layer")&&
(va=!1),Da&&va&&line.includes("<Name>")&&(nombre=line.substring(line.indexOf(">")+1,line.indexOf("</")),ua.push(nombre))},error:function(Da,va,Fa){},cache:!1})},T=function(ma){return"<span class='autowms_app_fileitem' name='"+ma+"'>&#128451;&nbsp;"+ma+"</span>"},R=function(ma,la){console.log("WMS",la);var ua="<span class='autowms_app_layeritem' title='Click to add to viewer' name='"+ma.name+"'><span class='autowms_app_layeritem_text'>Layer "+ma.text+" - ("+ma.name+")</span>";return ua+="<span class='autowms_app_layeritem_image'><img src=\""+
(la+"&service=WMS&request=getmap&format=image/png&layers="+ma.name+"&width=400&CRS=EPSG:4326&STYLES=&EXCEPTIONS=INIMAGE&showlegend=true&"+Math.random())+'" /></span></span>'},O=function(ma){var la="<span class='autowms_app_currentpath_hipa'>Project: <span class='autowms_app_request_container'><span class='dropdown'>  <button class='dropbtn'>Search</button> <span id=\"myDropdown\" class=\"dropdown-content\">";for(var ua=0;ua<L.length;ua++)la+='<a href="index.'+L[ua]+'.html">'+L[ua]+"</a>";la=la+'</span></span><input class=\'autowms_app_request_input\' type="text" value="'+
(hipatiaURL+'"></input>');la=la+"</span><hr/><span class='autowms_app_currentpath'><a href=\"/adaguc-viewer/hipatia_dw/"+(hipatiaURL+'/" target="_blank"');ma&&""!=ma&&"undefined"!==ma||(ma="");la+=">Carpeta: "+hipatiaURL+"</a>";"</span>";console.log(la);return la+('<span class="autowms_app_fileitem_header"><span class="autowms_app_fileitem_return">&#128193;&nbsp;<b>../ (&#8679;)</b></span><span class="autowms_app_fileitem_refresh" name="'+ma+'">&#8635;&nbsp;<b>Refresh</b></span></span>')},ba=function(ma){c.html("... working ... ");
var la=serverURL+"dataset="+hipatiaURL;console.log("SERVICES",la);var ua=WMJSGetServiceFromStore(la,xml2jsonrequestURL),Da=[];G("hipatia/"+hipatiaURL+".xml",ma,Da);ua.getLayerObjectsFlat(function(va){var Fa=""+O(hipatiaURL+"/"+ma);Fa+="</span>";c.html("... working ... ");w={};for(var Ma=0;Ma<va.length;Ma++)Da.includes(va[Ma].name)&&(console.log(va[Ma].name),Fa+=R(va[Ma],la),w[va[Ma].name]=va[Ma]);console.log("LAYER",w);c.html(Fa);$(".autowms_app_layeritem").attr("onclick","").click(function(Ka){Ka=
w[$(this).attr("name")];window.location.hash="addlayer('"+la+"&','"+Ka.name+"')"});$(".autowms_app_fileitem_return").attr("onclick","").click(function(Ka){ia("")});$(".autowms_app_request_button").attr("onclick","").click(function(Ka){ia("")});$(".autowms_app_request_input").keyup(function(Ka){13==Ka.keyCode&&ia("")})},function(va){console.log("HOLA");va=O();va=va+"<span class='autowms_app_error_container'><span class='autowms_app_error_container_header'>Unable to get WMS GetCapabilities</span><a target='_blank' href=\""+
(la+'&service=WMS&request=GetCapabilities">'+la+"</a>");c.html(va+"</span>");$(".autowms_app_fileitem_return").attr("onclick","").click(function(Fa){ia("")})},!0)},ia=function(ma){ma&&"undefined"!==ma||(ma=hipatiaURL&&"undefined"!==hipatiaURL?hipatiaURL:L[0]);var la=$(".autowms_app_request_input").first().val();la&&0<la.length&&(hipatiaURL=la);la=function(va){if(va.error)c.html("Error from server: "+va.error);else{w={};S=[];lines=va.split("\n");for(va=0;va<lines.length;va++)line=lines[va],line.includes("BEGIN ")&&
(folder=line.substring(line.indexOf("BEGIN ")+6,line.indexOf("--\x3e")),S.push(folder));var Fa=O(ma);c.html("... working ... ");S.sort(function(Ma,Ka){if(Ma.name<=Ka.name)return-1;if(Ma.name>Ka.name)return 1;if(Ma.name==Ka.name)return 0});for(va=0;va<S.length;va++)Fa+=T(S[va]),w[S[va]]=S[va];c.html(Fa);$(".autowms_app_fileitem_return").attr("onclick","").click(function(Ma){ia("")});$(".autowms_app_fileitem_refresh").attr("onclick","").click(function(Ma){var Ka=$(this).attr("name");$(".autowms_app_fileitem").remove();
$(".autowms_app_layeritem").remove();setTimeout(function(){ia(Ka)},100)});$(".autowms_app_fileitem").attr("onclick","").click(function(Ma){Ma=w[$(this).attr("name")];console.log("FILEOBJ",Ma);ba(Ma)});$(".autowms_app_request_button").attr("onclick","").click(function(Ma){ia(hipatiaURL)});$(".autowms_app_request_input").keyup(function(Ma){13==Ma.keyCode&&ia(hipatiaURL)})}};var ua=y+"request=getfiles&path="+ma,Da="hipatia/"+hipatiaURL+".xml";c.html('... Reading <a href="'+ua+'">'+ua+"</a> ... ");try{$.ajax({url:Da,
dataType:"text",success:la,error:function(va,Fa,Ma){console.error("Request failed without jsonp, trying jsonp");P("Unable extract data list")},cache:!1})}catch(va){console.error(va),P(va)}};this.enable=function(){c.html("<div>Initializing...</div>");x("./hipatia")};this.disable=function(){};this.resize=function(ma,la){}};try{WMJSExtApplications.Hipatia=hipatia_app}catch(c){};
