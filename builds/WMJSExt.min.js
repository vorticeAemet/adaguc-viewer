...
95;c.timer.init(100,c.fadeOut)})}},show:{fn:function(){c.getEl().dom.style.opacity=1;c.opacity=100}}}});this.callParent(arguments)}});initWMJS();var error=function(c){console.error(c)},debug=function(c){console.log(c)},versionInfo="ADAGUC viewer version AEMET 1.2.5.9",mainWebmapJS;function decimalFormatter(c,n){c=parseInt(c)+"";n=parseInt(n)-c.length;var x,y="";for(x=0;x<n;x++)y+="0";return y+c}
...
R("")};this.disable=function(){};this.resize=function(O,ba){}};try{WMJSExtApplications.AutoWMS=autowms_app}catch(c){};var hipatia_app=function(c,n){function x(ma){$.ajax({url:"./hipatia/project.ls",type:"GET",dataType:"text",success:function(la){datalist=la.split("\n");for(la=0;la<datalist.length;la++)""!=datalist[la]&&L.push(datalist[la].trim());hipatiaURL&&""!==hipatiaURL||(hipatiaURL=L[0]);ia("")},error:function(la,ua,Da){console.log(Da)},cache:!1})}initWMJS();c.addClass("autowms_app_container");var y="https://geoservices.knmi.nl/autowms?";try{y=autowmsURL,serverURL=autowmsURL.replace("autowms","adaguc-server")}catch(ma){}var w=
{},S=[],L=[],P=function(ma){var la=O();c.html(la+JSON.stringify(ma));$(".autowms_app_fileitem_return").attr("onclick","").click(function(ua){ia("")})},G=function(ma,la,ua){console.log("FOLD",la);$.ajax({url:ma,dataType:"text",success:function(Da){lines=Da.split("\n");for(var va=Da=!1,Fa=0;Fa<lines.length;Fa++)line=lines[Fa],line.includes("BEGIN "+la)&&(Da=!0),line.includes("END")&&(Da=!1),line.includes("<Layer")&&(va=!0),line.includes("</Layer")&&(va=!1),Da&&va&&line.includes("<Name>")&&(nombre=line.substring(line.indexOf(">")+
1,line.indexOf("</")),ua.push(nombre))},error:function(Da,va,Fa){},cache:!1})},T=function(ma){return"<span class='autowms_app_fileitem' name='"+ma+"'>&#128451;&nbsp;"+ma+"</span>"},R=function(ma,la){console.log("WMS",la);var ua="<span class='autowms_app_layeritem' title='Click to add to viewer' name='"+ma.name+"'><span class='autowms_app_layeritem_text'>Layer "+ma.text+" - ("+ma.name+")</span>";return ua+="<span class='autowms_app_layeritem_image'><img src=\""+(la+"&service=WMS&request=getmap&format=image/png&layers="+
ma.name+"&width=400&CRS=EPSG:4326&STYLES=&EXCEPTIONS=INIMAGE&showlegend=true&"+Math.random())+'" /></span></span>'},O=function(ma){var la="<span class='autowms_app_currentpath_hipa'>Project: <span class='autowms_app_request_container'><span class='dropdown'>  <button class='dropbtn'>Search</button> <span id=\"myDropdown\" class=\"dropdown-content\">";for(var ua=0;ua<L.length;ua++)la+='<a href="index.'+L[ua]+'.html">'+L[ua]+"</a>";la=la+'</span></span><input class=\'autowms_app_request_input\' type="text" value="'+
(hipatiaURL+'"></input>');la=la+"</span><hr/><span class='autowms_app_currentpath'><a href=\"/adaguc-viewer/hipatia_dw/"+hipatiaURL;ma&&""!=ma&&"undefined"!==ma||(ma="");la+=ma+'">Carpeta: '+hipatiaURL+"</a>";"</span>";console.log(la);return la+('<span class="autowms_app_fileitem_header"><span class="autowms_app_fileitem_return">&#128193;&nbsp;<b>../ (&#8679;)</b></span><span class="autowms_app_fileitem_refresh" name="'+ma+'">&#8635;&nbsp;<b>Refresh</b></span></span>')},ba=function(ma){c.html("... working ... ");
