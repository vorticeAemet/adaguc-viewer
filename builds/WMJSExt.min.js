...
95;c.timer.init(100,c.fadeOut)})}},show:{fn:function(){c.getEl().dom.style.opacity=1;c.opacity=100}}}});this.callParent(arguments)}});initWMJS();var error=function(c){console.error(c)},debug=function(c){console.log(c)},versionInfo="ADAGUC viewer version AEMET 1.2.5.6",mainWebmapJS;function decimalFormatter(c,n){c=parseInt(c)+"";n=parseInt(n)-c.length;var x,y="";for(x=0;x<n;x++)y+="0";return y+c}
...
ba="";0==n.layers.length&&(ba="No layer: Load a valid layer.",document.getElementById("info").innerHTML=ba);ia=[];for(la=0;la<n.layers.length;la++)ma=n.layers[n.layers.length-la-1].service,ua=n.layers[n.layers.length-la-1].name,ma.includes("TEMP")||ma.includes("KRR")||ma.includes("KRF")||ma.includes("ASV_FCT")||"sond_station"==ua?ia.push(n.layers[n.layers.length-la-1]):ma.includes("ECMWF")||ma.includes("HARMONIE")?ia.push(n.layers[n.layers.length-la-1]):ua.includes("sond_station")&&(console.log("ENTRO POR SOND_STATION"),
ia.push(n.layers[n.layers.length-la-1]));0==ia.length&&(ba="No Sounding valid Layer: Load a valid layer.",document.getElementById("info").innerHTML=ba);Da={};for(va in ia){console.log(va,ia.length);if(ia[va].service.includes("HARMONIE")||ia[va].service.includes("ECMWF")||ia[va].service.includes("TEMP")||ia[va].service.includes("KRR")||ia[va].service.includes("KRF")||ia[va].service.includes("ASV_FCT")||ia[va].name.includes("sond_station"))console.log("MYLAYER_TEXT",ia[va].name),Da.$jscomp$loop$prop$myLayer$277$344=
...
ma.queryable)n.featureInfoRequestReady("Layer is not queryable.",ma);else if(ma.name.includes("2D")||ma.name.includes("3D")){var ua=ia[la];ba.$jscomp$loop$prop$path$347="";ba.$jscomp$loop$prop$rad$348=ua.name.substr(-3,3);ba.$jscomp$loop$prop$dim$346="2D";ua.name.includes("3D")&&(ba.$jscomp$loop$prop$dim$346="3D");ma=WMJScheckURL(ua.service).split("/");var Da=ma[0];ma=ma[2];ba.$jscomp$loop$prop$path$347=Da+"//"+ma+"/";getJSONdate(ua,n,"text/plain",function(va){return function(Fa){var Ma=Fa.substring(0,
