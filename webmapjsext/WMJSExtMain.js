...
var versionInfo = "ADAGUC viewer version AEMET 1.2.5.7";
var mainWebmapJS;
...
function Console(div) {
  ...
  this.println = function (text, encode) {
    ...
    if (consoleData.length > consoleBufSize)
      consoleData = consoleData.substring(
        consoleData.length - consoleBufSize * (3 / 4)
      );
  };
...
var myUTCDateFormatter = function () {
  ...
  var iso =
    decimalFormatter(date.getFullYear(), 4) +
    "-" +
...
var myUTCTimeFormatter = function () {
  ...
  var iso =
    decimalFormatter(this.getUTCHours(), 2) +
    ":" +
...
var removeLayer = function () {
  ...
  if (layerObjects.length > 0) {
    if (lastIndex != -2) {
      ...
      if (lastIndex > layerObjects.length - 1)
        lastIndex = layerObjects.length - 1;
...
var createNewLayerPanel = function (webMapJS, config) {
  ...
  if (isDefined(config.service)) servicename = config.service;
  var layername = undefined;
  if (isDefined(config.name)) layername = config.name;
  var style = undefined;
  if (isDefined(config.style)) style = config.style;

  var opacity = undefined;
  if (isDefined(config.opacity)) {
    if (("" + config.opacity).length > 0) opacity = config.opacity;
  }
  var enabled = undefined;
  if (isDefined(config.enabled)) enabled = config.enabled;
    ...
    afterlayerready: function (layerPanel) {
      ...
      if (config.ready) {
        ...
        if (config.zoomtolayer == true) {
          ...
          webMapJS.setProjection(
            layerPanel.WMJSLayer.getProjection("EPSG:4326")
          );
        }
        //alert(173);
  ...
  listeners: {
    afterrender: {
      fn: function (t) {
        ...
        var timeselector = new WMJSTimeSelector(
          $(this.getEl().dom).find(".timeselector"),
          wmjstimecallback
        );
...
Ext.onReady(function () {
  ...
  var stopDate = Ext.create("Ext.form.field.Text", {
  ...
  var eastPanelGFI = Ext.create("Ext.panel.Panel", {
    ...
    listeners: {
      show: {
        fn: function () {
          eastPanelGFI
            .getDockedComponent(0)
            .getComponent("paneltitle")
            .setText(
              "App '" + eastPanelGFI.applicationSettings.name ||
                eastPanelGFI.applicationSettings.title + "'"
            );
          eastPanelGFI.update(
            '<div id="webmapjs_customapplication" class="webmapjs_customapplication"></div>'
          );
          ...
          if (!APP) {
            alert(
              "Application with name " +
                eastPanelGFI.applicationSettings.name +
                " not registered"
            );
            return;
          }
          eastPanelGFI.applicationSettings.app = new APP(
            $("#webmapjs_customapplication"),
            mainWebmapJS.webMapJS
          );
          eastPanelGFI.applicationSettings.app.enable(
            eastPanelGFI.width,
            mainWebmapJS.webMapJS.getHeight() - 24
          );
        },
      },
      hide: {
      ...
      resize: {
        fn: function () {
          if (eastPanelGFI.applicationSettings.app) {
            eastPanelGFI.applicationSettings.app.resize(
              eastPanelGFI.width,
              mainWebmapJS.webMapJS.getHeight()
            );
          }
        },
      },
    ...
    var panel = createNewLayerPanel(mainWebmapJS.webMapJS, {
      ...
      ready: function (layer, webmapjs) {
        ...
        if (baselayername && baselayerservice) {
          ...
          //alert(97);
          mainWebmapJS.baseLayer.parseLayer(
            function () {
              webmapjs.draw("dataPanelClicked::parseLayer");
            },
            undefined,
            "WMJSExtMain::dataPanelClicked"
          );
        } else {
          //alert(100);
  ...
  var maptypeclicked = function (node) {
    mainWebmapJS.webMapJS.setProjection({
      bbox: node.get("bbox"),
      srs: node.get("srs"),
    });
  ...
  var showPermaLinkWindow = function (webmapJS) {
    permaLinkWindow = Ext.create("Ext.window.Window", {
      ...
      modal: false,
      items: [
        Ext.create("webmapjsext.WMJSExt.PermaLinkPanel", {
          mappanel: mainWebmapJS,
        }),
      ],
    });
  ...
  var mainOptionMenuItems = [
    {
      text: I18n.show_time_selection_window.text,
    ...
    {
      text: I18n.create_animation.text + "&hellip;",
      iconCls: "button_settings_icon",
      handler: function () {
    ...
    },
    {
      ...
      handler: function () {
        showPermaLinkWindow(maptypeclicked);
      },
    },
    /*{
  ...
  for (var j = 0; j < getFeatureInfoApplications.length; j++) {
    ...
    if (
      getFeatureInfoApplications.open &&
      location.name === getFeatureInfoApplications.open
    ) {
      eastPanelGFI.setApplication(location);
    }
  }
  ...
  var projectionchooserpanel = Ext.create(
    "webmapjsext.WMJSExt.MapTypeSelector",
    { maptypeclicked: maptypeclicked, webmapjsext: mainWebmapJS }
  );
  var basemapselectorpanel = Ext.create("webmapjsext.WMJSExt.BaseMapSelector", {
    webmapjsext: mainWebmapJS,
  });
  var layersandmaps = {
    layout: "fit",
    border: false,
  ...
  var viewport = Ext.create("Ext.Viewport", {
    ...
    items: [viewportwestpanel, contentPanel1],
    ...
    listeners: {
      resize: {
        fn: function () {
          ...
          try {
            var bottom =
              viewportwestpanelwindow.y + viewportwestpanelwindow.height;
            if (bottom > viewport.height) {
              bottom = viewport.height - 70;
              if (bottom < 70) bottom = 70;
            }
            viewportwestpanelwindow.setSize(
              viewportwestpanelwindow.width,
              bottom
            );
          } catch (e) {}
        },
  ...
  var hashTagHasChanged = function (identifier, hashUrlVars) {
    if (identifier.indexOf("addlayer('") >= 0) {
      var service = identifier.substring(
        identifier.indexOf("addlayer('") + 10,
        identifier.indexOf("',")
      );
      var name = identifier.substring(
        identifier.indexOf(",'") + 2,
        identifier.indexOf("')")
      );
      //alert(service);
      ...
      if (service && name) {
        ...
        for (var j = 0; j < layersAvailable.length; j++) {
          ...
          if (
            layersAvailable[j].service == service &&
            layersAvailable[j].name == name
          ) {
      ...
      mainWebmapJS.webMapJS.positionMapPinByLatLon({
  ...
  var checkIfClassicADAGUCStateIsGiven = function (urlVars) {
    ...
    if (isDefined(urlVars.collapse)) {
      if (urlVars.collapse == "1") {
        Ext.getCmp("viewportwestpanel").collapse();
      }
    }

    if (isDefined(urlVars.zoomtolayer)) {
      if (urlVars.zoomtolayer == "1") {
        zoomToLayer = true;
      }
    }
    ...
    if (urlVars && urlVars.autowms) {
      autowmsURL = urlVars.autowms + "";
      if (autowmsURL.indexOf("?") === -1) {
        autowmsURL += "?";
      }
      window.setTimeout(() => {
        eastPanelGFI.setApplication(
          getFeatureInfoApplications.filter((a) => a.name === "AutoWMS")[0]
        );
        eastPanelGFI.hide();
        eastPanelGFI.show();
      }, 100);
